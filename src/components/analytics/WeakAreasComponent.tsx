...
import {
  Brain, Sparkles, Target, RefreshCw, AlertTriangle,
...
interface QuizAttemptSummary {
  ...
  startedAt: string;
  weakAreaAnalysis: AttemptAnalysis | null;
...
export default function WeakAreasComponent({ isLoading: externalLoading = false }: WeakAreasComponentProps) {
  ...
  useEffect(() => { if (user) fetchAttempts(); }, [user]);
  ...
  const fetchAttempts = async () => {
    ...
    try {
      ...
      await Promise.all((quizzes || []).map(async (quiz: { id: string; title: string; difficulty: number }) => {
        const attRes = await fetch(`/api/quizzes/${quiz.id}/attempts`, { headers: { Authorization: `Bearer ${token}` } });
        if (!attRes.ok) return;
        const data = await attRes.json();
        (Array.isArray(data) ? data : []).filter((a: QuizAttemptSummary) => a.completedAt !== null)
  ...
  const handleAnalyze = async (attemptId: string) => {
    ...
    try {
      ...
      // Update the attempt in state with the saved analysis
      setAttempts((prev) => prev.map((a) => a.id === attemptId ? { ...a, weakAreaAnalysis: data.analysis } : a));
  ...
  if (loading || externalLoading) return <div className="space-y-3 animate-pulse">{[1,2,3].map(i=><div key={i} className="h-16 bg-slate-700/50 rounded-lg"/>)}</div>;
  if (error) return <Card className="bg-slate-900/50 border-slate-700/50"><CardContent className="p-10 text-center"><AlertTriangle className="w-10 h-10 text-red-400 mx-auto mb-3"/><p className="text-slate-400 text-sm mb-4">{error}</p><Button onClick={fetchAttempts} variant="outline" className="bg-slate-800/40 border-slate-600/50 text-slate-200"><RefreshCw className="w-4 h-4 mr-2"/>Try Again</Button></CardContent></Card>;
  if (attempts.length === 0) return <Card className="bg-slate-900/50 border-slate-700/50"><CardContent className="p-10 text-center"><Target className="w-10 h-10 text-slate-600 mx-auto mb-3"/><p className="text-slate-300 font-medium mb-1">No quiz attempts yet</p><p className="text-slate-500 text-sm">Complete some quizzes to see your weak area analysis here.</p></CardContent></Card>;

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between mb-1">
        <p className="text-slate-400 text-sm">{attempts.length} completed attempt{attempts.length !== 1 ? "s" : ""}</p>
        <Button onClick={fetchAttempts} variant="ghost" size="sm" className="text-slate-400 hover:text-cyan-400 h-7 w-7 p-0"><RefreshCw className="w-3.5 h-3.5"/></Button>
      </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className={cn("px-3 py-1.5 rounded-lg border text-sm font-bold flex-shrink-0", scoreBadge(attempt.score))}>{Math.round(attempt.score)}%</div>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {wrongCount > 0 && !analysis && (
                    <Button size="sm" onClick={() => handleAnalyze(attempt.id)} disabled={isAnalyzing}
                      className="bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white h-7 px-3 text-xs">
                      {isAnalyzing ? <><Loader2 className="w-3 h-3 mr-1 animate-spin"/>Analyzing</> : <><Sparkles className="w-3 h-3 mr-1"/>Analyze</>}
                    </Button>
                  )}
                  {analysis && (
                    <Button size="sm" variant="ghost" onClick={() => setExpandedAttempt(isExpanded ? null : attempt.id)}
                      className="text-violet-400 hover:text-violet-300 text-xs h-7 px-2 flex items-center gap-1">
                      <Brain className="w-3 h-3"/>Analysis{isExpanded ? <ChevronUp className="w-3 h-3"/> : <ChevronDown className="w-3 h-3"/>}
                    </Button>
                  )}
                </div>
              </div>
              ...
              {analysis && isExpanded && (
                <div className="mt-4 pt-4 border-t border-slate-700/50 space-y-3">
                  ...
                  {analysis.weakTopics.length > 0 ? (
                    <>
                      <div><p className="text-slate-500 text-xs uppercase tracking-wide mb-2">Topics to review</p><div className="flex flex-wrap gap-2">{analysis.weakTopics.map((t,i)=><span key={i} className="px-2.5 py-1 bg-orange-500/15 border border-orange-500/30 text-orange-300 rounded-full text-xs">{t}</span>)}</div></div>
                      <div><p className="text-slate-500 text-xs uppercase tracking-wide mb-2">Recommendations</p><ol className="space-y-1.5">{analysis.recommendations.map((r,i)=><li key={i} className="flex items-start gap-2 text-xs text-slate-300"><span className="text-violet-400 font-bold flex-shrink-0">{i+1}.</span>{r}</li>)}</ol></div>
                    </>
